...
export default function LocationInputs({ cityCaption, theatreCaption, user, defaults }: {
    ...
    const [nearbyTheatres, setNearbyTheatres] = useState<string[]>(
        (locationDefaults?.city && locationDefaults?.state) ? getTheatresByCity(locationDefaults.city, locationDefaults.state, 0).map((t) => t.name) : []
    );
    ...
    const inputTheatres = locationDefaults?.theatres ? (
        locationDefaults.theatres.filter((theatre) => !nearbyTheatres.includes(theatre))
    ) : [];
    return <div>
                ...
                {nearbyTheatres.map((theatre, i) => (
                    <li key={i} className="m-3">
                        <Checkbox
                            ...
                            label={theatre}
                            value={theatre}
                            defaultChecked={locationDefaults?.theatres?.includes(theatre)}
                        />
                    </li>
                ))}
            </ul>
