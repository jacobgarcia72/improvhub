...
export default async function EventDetailsPage({ id, type }: { id: string, type: EventType }) {
    ...
    const instructorTitle = type === 'jam' ? 'Host' : 'Instructor';
    ...
    return <>
        <EventHeader event={event}>{onlyOneOccurrence ? <EventDate type={type} eventDate={occurrences[0].dateTime} /> : <h3 className="font-semibold font-lg pt-2 pb-0">{type === 'class' ? `${occurrences?.length}-Day Class` : `${capType} Series`}</h3>}</EventHeader>
        ...
        {isAdmin || isInstructor ? <div className="px-3 pb-3 mt-3 bg-gray-800/20 border border-gray-500 rounded">
            <div className="flex justify-center"><h2 className="my-2">{isAdmin ? 'Admin Panel' : `${instructorTitle} Panel`}</h2></div>
                ...
                {['jam', 'class', 'workshop'].includes(type) && (
                        <div>
                            <Link href={`/${pluralize(type)}/${id}/instructors`}>
                                <Button caption={`Manage ${pluralize(instructorTitle)}`} className="w-54" />
                            </Link>
                        </div>
                )}
                <div>
